--[[
    ...
    -- THIS IS AN UNRELEASED AND UNFINISHED VERSION, AND AS SUCH CAN BE BUGGY --
...
local function setMusicPathForSection(section,path)
    return mem(customMusicPathsTbl + section*4, FIELD_STRING, path)
end
...
local function blockSpawnWithSizeableOrdering(id,x,y)
    local v = Block.spawn(math.clamp(id,1,BLOCK_MAX_ID),x,y)

    if id <= 0 or id > BLOCK_MAX_ID then -- Account for block ID of 0
        v.id = id
        v.width,v.height = 32,32
    end
    ...
    if Block.config[v.id] and Block.config[v.id].sizeable then -- If this block is a sizeable
        ...
        -- Acount for some weird stuff that'll be fixed in patch 2
        if not Block.SEMISOLID_MAP[v.id] then
            sizeableCount = sizeableCount + 1
            mem(GM_SIZABLE_COUNT_ADDR,FIELD_WORD,sizeableCount)
            mem(GM_SIZABLE_LIST_ADDR+(sizeableCount*0x02),FIELD_WORD,v.idx)
        end
...
rooms.classesToSave = {
    ...
    },
    {
        ...
        saveFields = {
            ...
            "despawnTimer",{0x124,FIELD_BOOL}, -- Despawning related stuff
        },
        ...
        extraRestore = (function(v,fields)
            if v.despawnTimer <= 0 and fields[0x124] then
                v.despawnTimer = 1
            end
    ...
    },
    {
        ...
        saveFields = {
            "boundary","origBoundary","musicID","wrapH","wrapV","hasOffscreenExit","backgroundID","origBackgroundID",
            "noTurnBack","isUnderwater","settings",
        },

        extraSave    = (function(v,fields) fields.musicPath = getMusicPathForSection(v.idx) end),
        extraRestore = (function(v,fields)
            setMusicPathForSection(v.idx,fields.musicPath)

            if player.section == v.idx then
                playMusic(v.idx)
            end
        end),
    },
...
function rooms.onStart()
    ...
    for k,v in ipairs(possibleMusicIniPaths) do
        if io.exists(v.. "\\music.ini") then -- If there's a music.ini file here
            for _,w in ipairs(parseWithHeaders(v.. "\\music.ini",{})) do -- Go through each entry
                if w._header and w.file then -- If this entry is actually valid
                    local filename = w.file
                    local metadataStart = filename:find("|") -- Get where the metadata for the file starts

                    if metadataStart then -- If this file has some metadeta
                        filename = filename:sub(1,metadataStart-1) -- Cut out the metadeta
                    end

                    if io.exists(v.. "\\".. filename) then -- If the file actually exists
