...
do
    ...
    local function doorTransitionEffects()
        if warpTransition.currentWarp.warpType == 2 then
            player.frame = 1
            SFX.play(46)
        end
    end
...
function warpTransition.onTick()
    ...
    if warpTransition.currentWarp ~= nil then
        if warpTransition.currentWarp.entranceSection == warpTransition.currentWarp.exitSection and warpTransition.currentWarp.levelFilename == "" and not warpTransition.currentWarp:mem(0x84,FIELD_BOOL) then
...
function warpTransition.onCameraDraw(camIdx)
    ...
    if warpTransition.currentTransitionType ~= warpTransition.TRANSITION_NONE then
        ...
        if warpTransition.currentTransitionType ~= warpTransition.TRANSITION_NONE and warpTransition.musicFadeOut and exitHasDifferentMusic(warpTransition.currentWarp) then
            -- Music fade out
            if player.section == warpTransition.currentWarp.entranceSection and (middle ~= nil and middle ~= 0) then
...
warpTransition.crossSectionTransition = warpTransition.TRANSITION_SWIRL
...
warpTransition.levelStartTransition = warpTransition.TRANSITION_SWIRL
...
warpTransition.transitionSpeeds = {
    ...
    [warpTransition.TRANSITION_SWIRL    ] = 64, -- How many frames it takes to complete the transition.
}
